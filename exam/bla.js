...
async function main() {
    ...
    function renderPageButtons(currentPage,dataLength ) {
        ...
        for (var i = currentPage; i < currentPage + 5; i++) {
            if (i <= Math.ceil(dataLength / perPage)) {
                ...
                if (selectedBtn2 !== null) {//для сохранения прожатия кнопки в таблицу с гидами
                    document.getElementById('btn2' + selectedBtn2).click();
                }
    ...
    async function applyFiltersAndRender() {//для селекта с гидами и выбором их опыта работы
        ...
        const tableData2 = await getListOfGuides(selectedBtn);//запрашиваем таблицу гидов в id маршрута
